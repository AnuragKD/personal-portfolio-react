...
const socialLinks = [
  {
    name: 'GitHub', href: 'https://github.com/AnuragKD',
    icon: (
  ...
  {
    name: 'LinkedIn', href: 'https://www.linkedin.com/in/anurag-kd-3a3027202',
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
...
const ContactForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const accessKey = "8f2cd3ce-4b4f-45c4-a7c2-bb44be227295"
            ...
            {submitted ? (
              ...
              <Card variant="default" hover={false} className="p-6 shadow-sm">
                <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                  <InputField required label="Your Name" id="name" placeholder="Anurag Kumar" value={form.name} onChange={handleChange('name')} />
                  <InputField required label="Email Address" id="email" type="email" placeholder="anurag@example.com" value={form.email} onChange={handleChange('email')} />
                  <InputField required label="Message" id="message" placeholder="Tell me about your project..." rows={5} value={form.message} onChange={handleChange('message')} />
                  <button type="submit"
