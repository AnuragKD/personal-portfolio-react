    ...
    <header className="w-full fixed top-0 z-50">
      <div className="max-w-7xl mx-auto px-5 md:px-10">
        <nav className=" relative my-3 border-1 border-gray-200 rounded-full py-3 px-4 flex justify-between items-center">
          ...
          {/* Desktop Menu */}
          <div className="hidden lg:flex gap-1 px-1 py-1 bg-white/80 border-1 border-gray-300 rounded-full">
            {navLinks.map((link) => (
              <NavLink
                key={link.name}
                to={link.path}
                className={({isActive}) => 
                  `text-sm px-4 py-2 rounded-full text-black font-normal font-primary-font transition ${
                      isActive 
                      ? "border-1 border-primary-color bg-primary-color text-white hover:bg-primary-color/90" 
                      : " "
                  }`}
              >
                {link.name}
          ...
          <div className="lg:hidden relative">
              ...
              >
                  <div className="flex flex-col items-end gap-2 p-4 pt-13 rounded-[20px] bg-white/50 backdrop-blur-lg border-1 border-gray-200">
                  {navLinks.map((link) => (
                      <NavLink
                      key={link.name}
                      to={link.path}
